...
use socket_factory::{DatagramIn, DatagramOut, DatagramSegmentIter, SocketFactory, UdpSocket};
use std::{
    io,
...
impl Sockets {
    ...
    pub fn send(&mut self, datagram: DatagramOut<'static>) -> io::Result<()> {
...
struct ThreadedUdpSocket {
    outbound_tx: flume::r#async::SendSink<'static, DatagramOut<'static>>,
...
impl ThreadedUdpSocket {
    ...
    fn send(&mut self, datagram: DatagramOut<'static>) -> io::Result<()> {
        self.outbound_tx
